...
3. If an attempt was already evaluated, return the existing result without calling the LLM again. Support a `force` flag to re-evaluate when anchors or prompts change.
...
2. [ ] Create `backend/src/routes/evaluate.py` with `POST /api/v1/attempts/{attempt_id}/evaluate` (accepts optional query param `force: bool = False`):
   ...
   - Step 3: If `force=False`, query `evaluations` table by attempt_id; if row exists, query `eval_scores` for that attempt and return existing `EvaluationResponse`. If `force=True`, delete existing `eval_scores` and `evaluations` rows for this attempt and proceed to step 4.
   ...
   - Step 8: Measure wall-clock time of `evaluate_answer()` call
   - Step 9: Insert one row into `evaluations` with raw_response, model, prompt_tokens, completion_tokens, and wall_clock_ms
   - Step 10: Return `EvaluationResponse`
...
4. Idempotency: call evaluate twice without `force`, verify second call returns same scores and `evaluate_answer` was called only once
5. Force re-evaluation: call evaluate with `force=true` after first evaluation, verify `evaluate_answer` is called again and old scores are replaced
6. Response `model` and `created_at` fields are populated
